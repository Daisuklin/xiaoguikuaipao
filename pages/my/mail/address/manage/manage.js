// pages/my/mail/address/manage/manage.js
var app = getApp();
...
Page({
  ...
  manageList: function () {
    ...
    var url = appUtil.ajaxUrls().addressUrl;
    that.setData({
      url: url
    });
    wx.showLoading({
  ...
  jump(e) {
    ...
    //保存至本地缓存
    wx.setStorageSync('mailId', mailId);
    ...
    if (this.options.transmitId == 15626199190) {//来自店铺的入口
      if (length <= 7) {
        ...
        wx.setStorageSync('assignment', '0')
      } else {
        wx.navigateBack({
          delta: 2
        })
