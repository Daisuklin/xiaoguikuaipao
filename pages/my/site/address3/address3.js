// pages/my/site/address3/address3.js
var app = getApp();
...
Page({
  ...
  //编辑后保存
  saveButton(e) {
    var entrance = this.options.transmitId;//判断来自何方的入口
      ...
      wx.request({
        ...
        success: function (res) {
          ...
          if (res.data.succeeded == true) {
            ...
            } else if (appUtil.appUtils.getMemberIdData().userData.defaultAddress == id) {
              that.setDefaultAdressId(null);
              wx.removeStorageSync('dispute')
            }
            if (entrance == 15626199190) {//判断来自确认订单的入口，做相应的跳转方式
              wx.setStorageSync("defaltId", id);//将id储存到本地
              wx.hideLoading()
              wx.navigateBack({
                delta: 2
              })
    ...
    if (b == null || b == 0) {
      ...
      wx.request({
        ...
        success: function (res) {
          ...
          if (res.data.succeeded == true) {
            if (that.options.transmitId == 15626199190) {//判断来自确认订单的入口，做相应的跳转方式
              var through = res.data;
              var id = through.data.id;
              wx.setStorageSync("defaltId", id);//将id储存到本地
              wx.hideLoading()
